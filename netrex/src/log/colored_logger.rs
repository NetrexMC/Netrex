use crate::{ansii_yellow, ansii_aqua, ansii_red, ansii_gray};

use super::{Loggable, logger::Logger};
use super::{PREFIX_DEBUG, PREFIX_WARN, PREFIX_INFO, PREFIX_ERROR};
...
impl Loggable for ColoredLogger {
	...
	fn debug(&self, message: &str) {
		self.logger.log(ansii_gray!(format!("{} {}", PREFIX_DEBUG, message).as_str()).as_str());
	}

	fn warn(&self, message: &str) {
		self.logger.log(ansii_yellow!(format!("{} {}", PREFIX_WARN, message).as_str()).as_str());
	}

	fn info(&self, message: &str) {
		self.logger.log(ansii_aqua!(format!("{} {}", PREFIX_INFO, message).as_str()).as_str());
	}

	fn error(&self, message: &str) {
		self.logger.log(ansii_red!(format!("{} {}", PREFIX_ERROR, message).as_str()).as_str());
	}
}